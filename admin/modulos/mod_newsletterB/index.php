...
function popup (id) {
//	window.open("../newsletter/newsletter.php?id="+id,"Preview","scrollbars=yes,width=800,height=800");
	window.open("../newsletter/newsletter3/index.php?id="+id,"Preview","scrollbars=yes,width=800,height=800");
}

function enviar(estado, newlsetter) {
	$.ajax({
		  url: '/ajax/newsletter_estado.php',
		  type: 'POST',
		  async: true,
		  data: 'estado=' + estado + '&newsletter=' + newlsetter,
		  success: function() {
			location.reload();
          	  	  ...
          	  	  <table border="0">
          	  	    ...
          	  	    <tr>
          	  	      <td>
						   ...
						   <select name="orden">
						     <option value="nombre" <? if ($orden == "nombre") echo "selected";?>>Nombre</option>
						     <option value="fecha_modificacion" <? if ($orden == "fecha_modificacion") echo "selected";?>>Fecha</option>
						   </select>
						   <label> orden:</label>
        ...
        <tr>
          ...
          <form name="form_listado_seleccion" method="post" action="">
	          <div id="resultados">
				...
				?>
						  <tr>
			                <td class="<?=$estilo?>"><?=$newsletter['id']?></td>
			                <td class="<?=$estilo?>"><input type="checkbox" name="seleccion<?=$num_fila;?>" value="<?=$newsletter['id']?>" /></td>
			                <td class="<?=$estilo?>"><strong><?=htmlentities($newsletter['nombre'])?></strong></td>
			                <td class="<?=$estilo?>"><?=htmlentities($newsletter['tipo_newsletter'])?></td>
			                <td class="<?=$estilo?>"><?=date("H:i d-m-Y",$newsletter['hora_envio']);?></td>
			                <td class="<?=$estilo?>" align="center"><? if ($newsletter['preparada'] == 0) echo "<a href='?modulo=mod_newsletter&accion=estado&campo=preparada&fns=1&id=".$newsletter['id']."&page=".$page."'><img src='images/cross.png' border='0'/></a>"; else echo "<a href='?modulo=mod_newsletter&accion=estado&campo=preparada&fns=1&id=".$newsletter['id']."&page=".$page."'><img src='images/tick.png' border='0'/></a>";?></td>
...
//			                if ($newsletter['enviada'] == 0) echo "<img src='images/cross.png' border='0' onclick='enviar(false," . $newsletter['id'] . ");'/>";
			                if ($newsletter['enviada'] == 0) echo "<a href='/ajax/newsletter_estado.php?estado=false&newsletter=".$newsletter['id']."'><img src='images/cross.png' border='0'/></a>";
//			                else echo "<img src='images/tick.png' border='0' onclick='enviar(false," . $newsletter['id'] . ");'/>";
			                else echo "<a href='/ajax/newsletter_estado.php?estado=true&newsletter=".$newsletter['id']."'><img src='images/tick.png' border='0'/></a>";
			                ?>
			                </td>
			                <td class="<?=$estilo?>" align="right"><img src="images/preview_peque.png" border="0" alt="Previsualizar" title="Previsualizar" onclick="popup(<?=$newsletter['id']?>);" style="cursor:pointer"/><a href="?modulo=mod_newsletter&accion=editar&id=<?=$newsletter['id']?>"><img src="images/page_edit.png" alt="Editar" title="Editar" border="0" /></a></td>
			              </tr>
			    <?
			      		if($r%2==0)++$r;else--$r;
			        	}
