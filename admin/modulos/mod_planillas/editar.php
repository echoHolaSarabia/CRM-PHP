...
//  $elementos_planificables += $funciones->get_noticias();
  $elementos_planificables = array_merge($funciones->get_noticias(), $funciones->get_modulosrosas());
  $accion = "insert";
}
...
function muestra_columna($elementos, $num_columna, $estilos, $estilos_helper, $limite_inferior, $limite_superior, &$num) {
  if (!empty($elementos[$num_columna])) {
    ...
    foreach ($elementos[$num_columna] as $unElemento) {
      ...
      else if ($unElemento['tabla_elemento'] == "modulosrosas") {//Si es un módulo rosa tengo que averiguar su tipo para mostrarlo de uno u otro color
...
<?php /*
  ...
  <?phpfor ($i=1;$i<=NUM_SUBPLANILLAS;$i++){?>
  <td class="pestania_off" id="pestania<?php=$i?>" onclick="javascript:cambiar_capa('tabla_contenedor',<?php=$i?>,<?php=NUM_SUBPLANILLAS+1?>);"><div id="pp<?php=$i?>">Pestaña <?php=$i?></div></td>
  <?php}?>
  </tr>
  </table>
 */ ?>
  ...
  <tr>
    <td>
      <form name="form_planillas" method="post" action="?modulo=<?php echo MODULO ?>&fns=1&accion=<?php echo $accion ?>&seccion=<?php echo $_GET["seccion"] ?>">
        ...
        Fecha de publicación:<input type="text" name="fecha_publicacion" id="fecha_publicacion" value="<?php echo (isset($planilla)) ? $planilla['fecha_publicacion'] : date("Y-m-d H:i"); ?>" />&nbsp;&nbsp;<input type="button" class="txt" id="lanzador" value="..." />
        <div style="display: none;">
          Tipo de portada:
          <select name="tipo">
            <option value="con_rotador" <?php echo (isset($planilla) && ($planilla["tipo"] == "con_rotador")) ? " SELECTED " : "" ?>>Con rotador</option>
<?php /* 	<option value="sin_rotador" <?php=(isset($planilla) && ($planilla["tipo"]=="sin_rotador")) ? " SELECTED " : ""?>>Sin rotador</option>
  <option value="una_foto" <?php=(isset($planilla) && ($planilla["tipo"]=="una_foto")) ? " SELECTED " : ""?>>Una foto/Sección</option>
 */ ?>
          </select>
        </div>
      </form>
...
<?php for ($i = 1; $i <= NUM_SUBPLANILLAS; $i++) { ?>
    <table  border="0" style='display:<?php echo ($i < 4) ? "inline" : "none" ?>' id="tabla_contenedor<?php echo $i ?>">
      <tr>
        <td>
          <div>
            <div class="columna1">
              <h2><?php echo ($i == 1) ? "Rotador" : "Columna 1" ?></h2>
  ...
  <?php muestra_columna($elementos_planificados, 3, $estilos, $estilos_helper, $limite_inferior, $limite_superior, $num); ?>
            ...
            <div class="salto"></div>
            <div class="columna4" style="display:none">
              <h2>Columna 1 y 2</h2>
            ...
            <div class="columna5" style="display:none">
              <h2>Columna 2 y 3</h2>
          ...
          <ul id="origen<?php echo $i ?>" style="padding-left:0px;margin-left:0px;padding-bottom:10px;background-color:#EEEEEE;border:1px solid #CCCCCC">
            ...
            foreach ($elementos_planificables as $unElementoPlanificable) {
              if (!empty($elementos_planificables[$j - 1])) {
                /* echo '<br />elemento '.$j-1; */
                $estilo_helper = $estilos_helper[$tabla];
                if ($unElementoPlanificable['programado'] == 1)
                  $estilo = $estilos['programada'];
                else if ($unElementoPlanificable['tabla_elemento'] == "modulosrosas") {//Si es un módulo rosa tengo que averiguar su tipo para mostrarlo de uno u otro color
                ...
                /* else{
                  echo 'la tabla es otra';
                  print_r($unElementoPlanificable);
                  } */
              }/* else{
                echo 'empty'.$j;
                } */
...
?>
    ...
    <tr id="modulosnegros" style="display:none;">
      <td>
