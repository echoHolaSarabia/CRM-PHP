...
class Funciones {
  ...
  function insert($arr_datos) {
    ...
    if ($arr_datos["redireccion"] == -1)
      echo "";
    else if ($arr_datos["redireccion"] == 1)
...
//		die($query);
  ...
  function getNoticias($newsletter_id) {
    $resultado = array();
    $query = "SELECT n.id, n.img_vertical AS imagen, n.titulo, n.entradilla, n.texto, n.img_horizontal, s.titulo AS seccion, n.subseccion, nnr.orden1, nnr.orden2
    FROM noticias AS n, noticias_newsletter AS nn, noticias_newsletter_relacionadas AS nnr, secciones AS s
    WHERE nnr.id_elemento = nn.id
    AND nn.id_original = n.id
  ...
  function getMegaBanner($id) {
    ...
    if ($banner["fuente"] == "codigo") {
      ...
      <img src="http://' . $_SERVER['HTTP_HOST'] . '/userfiles/banners/' . $banner["imagen"] . '" alt="' . $banner["titulo"] . '" style="border: 0;" />
     </a>';
    }
  }
  ...
  function redireccionar($pagina = "") {
   ...
   //código MIO      ########################### En lugar de redireccionar el Index, hace una llamada para insert en phplist
	$sql_consulta = mysql_query('SELECT `id` FROM `newsletters` order by `id` desc LIMIT 1');
	while($row_consulta = mysql_fetch_array($sql_consulta)){
		$id_consulta = $row_consulta['id'];
	}
	header('location: http://www.padelspain.net/newsi.php?id='.$id_consulta);
	//fin código MIO  ###########################
...
//    if (isset($q) and !eregi('^ *$', $q)) {
  ...
  function get_noticias_newsletter_origen($id_newsletter, $tipo_newsletter) {
    if ($tipo_newsletter == "Newsletter")
      $tipo = "estado_1";
    else if ($tipo_newsletter == "Aquí España")
      $tipo = "estado_2";
    else if ($tipo_newsletter == "Club FEHR")
      $tipo = "estado_3";
    else if ($tipo_newsletter == "Confidencial")
      $tipo = "estado_4";
   ...
   */
	...
	function duplicar($arr_ids){
		foreach($arr_ids as $clave => $valor){
		  if ($clave != "seleccion"){//No incluyo el checkbox que selecciona todos y que tiene valor "on"
			...
			//echo "CONSULTA 1 DUPLICAR : SELECT * FROM ".$this->tabla." WHERE id=".$valor."</br>";
			
			$fila = mysql_fetch_assoc($r);
			
			// Obtenemos el id de todas las noticias y banners intertadas en la actual newsletter
			$cad = "";
			
			$noticias = mysql_query("SELECT id FROM noticias_newsletter_relacionadas  WHERE id_newsletter=".$valor);
			
			//echo "NOTICIAS: SELECT id FROM noticias_newsletter_relacionadas  WHERE id_newsletter=".$valor."</br>";
			...
			//echo "CADENA NOTICIAS : ".$cad."</br>";
			
			//print_r($fila);
			
			$this->insertNewsletter(array_slice($fila,1));
	...
	function insertNewsletter($datos){
		$c="INSERT INTO ".$this->tabla." ( id";
		foreach ($datos as $clave => $valor){
		  if ($clave != "noticias_relacionadas")
			$c.=",".$clave;
		}
		$c.=") VALUES (null";
		foreach ($datos as $clave => $valor){
		...
		foreach ($datos as $clave => $valor){
			if ($clave == "noticias_relacionadas"){
				$valor = trim($valor, ',');
				$noticias = explode(",",$valor);
				print_r($noticias);
			}
		}
		
		//echo "Total noticias: ".count($noticias);
		
		for($i=0; $i<count($noticias); $i++){
			$sql = "INSERT INTO noticias_newsletter_relacionadas (id_newsletter, id_elemento, tabla_elemento, orden1, orden2) 
					SELECT ".$id.", id_elemento, tabla_elemento, orden1, orden2 
					FROM noticias_newsletter_relacionadas WHERE id =".$noticias[$i];
