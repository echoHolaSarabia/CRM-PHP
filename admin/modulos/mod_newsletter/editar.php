...
<script type="text/javascript">
  $(document).ready(function(){
    $('#destino1').sortable({connectWith:'ul',cursor:'move'});
    $('#destino2').sortable({connectWith:'ul',cursor:'move'});
    $('#destino3').sortable({connectWith:'ul',cursor:'move'});
  });
  function previsualizar_newsletter(){
    if (document.form_newsletter.tipo_newsletter.value=="")
      ...
      url="../newsletters.php?id=<?php echo (isset($_GET["id"])) ? $_GET["id"] : "" ?>";
      // 		url="../last/index.php?id=<?php echo (isset($_GET["id"])) ? $_GET["id"] : "" ?>";
      window.open(url);
    }
  }
  ...
  function pruebatext() {
	...
	if(document.getElementById('suscrip').value != '') {
		
		document.getElementById('suscrip').value = 'Suscriptores Insertados con éxito';
  ...
  function cargar_noticias(obj,refresca_suscriptores){
    var tipo ="estado_1";
<?php foreach ($tipos_newsletter as $tipo) { ?>
            if (obj.value=="<?php echo $tipo[0] ?>") tipo ="<?php echo $tipo[1] ?>";
<?php } ?>
          ...
          datos = "id=<?php echo (isset($_GET["id"])) ? $_GET["id"] : "-1" ?>&tipo="+tipo
          $.ajax({url:url_destino,data:datos,success: function(datos){$("#noticias").html(datos)},type:"POST"});
        }
        function obtener_suscriptores(){
          ...
          if (tipo == "") alert("debe seleccionar un tipo de newsletter");
            ...
            var tipo ="n_1";
<?php foreach ($tipos_newsletter as $tipo) { ?>
                      if (tipo=="<?php echo $tipo[0] ?>") tipo ="<?php echo $tipo[2] ?>";
<?php } ?>
...
if (isset($_GET['id']) && $_GET['id'] != "") {
  ...
  $accion = "insert";
  //Saco el identificador de la última newsletter creada para obtener la posicion de usu banners
  ...
  <li id="item_<?php echo $id ?>,<?php echo $tabla ?>" style="<?php echo $estilo ?>" value="<?php echo $tabla ?>" title="<?php echo $tabla ?>">
    <div style="<?php echo $estilo_helper ?>">
      <table width="100%">
        <tr>
          <td></td>
  <!--				<td onclick="javascript:muestra_contenido(<?php echo $id ?>,'<?php echo $tabla ?>'<?php //=",".$num ?>)" style="cursor:pointer" align="right" id="abiertocerrado-<?php echo $tabla . "-" . $id ?>">[+]</td>-->
...
<tr>
  <td width="100%">
    <table width="100%" border="0" cellpadding="0" cellspacing="0">
      ...
      <tr>
        <td>
          <form id="form_newsletter" name="form_newsletter" method="post" action="?modulo=mod_newsletter&fns=1&accion=<?php echo $accion ?>" enctype="multipart/form-data">
            <input type="hidden" name="redireccion" id="redireccion" value="" />
            <input type="hidden" name="fecha_creacion" id="fecha_creacion" value="<?php echo (isset($newsletter)) ? htmlentities($newsletter['fecha_creacion']) : ""; ?>" />
                            ...
                            <td>
                              <table width="100%" border="0" cellpadding="0" cellspacing="0" >
                                <tr>
                                  ...
                                  <td><input type="text" name="nombre" value="<?php echo (isset($newsletter)) ? htmlentities($newsletter['nombre']) : ""; ?>" size="85" /></td>
                                </tr>
                              </table>
                            </td>
                          </tr>
                            ...
                            <td>
                              <table width="100%" border="0" cellpadding="0" cellspacing="0" >
                                <tr>
                                  ...
                                  <td><input type="text" name="asunto" value="<?php echo (isset($newsletter)) ? htmlentities($newsletter['asunto']) : ""; ?>" size="85" /></td>
                                </tr>
                              </table>
                            </td>
                          </tr>
                                ...
                                <tr style="display: none;">
                                  ...
                                  <td style="display: none;">
                                    <select name="tipo_newsletter" id="tipo_newsletter" style="width:448px" onchange="cargar_noticias(this,true)">
                                      <?php foreach ($tipos_newsletter as $tipo) { ?>
                                        <option value="<?php echo $tipo[0] ?>" <?php echo (isset($newsletter) && ($newsletter["tipo_newsletter"] == $tipo[0])) ? "selected" : "" ?>><?php echo $tipo[0] ?></option>
<?php } ?>
                                    </select>
                                  </td>
                                </tr>
                            ...
                            <td>
                              <table width="100%" border="0" cellpadding="0" cellspacing="0" >
                                <tr>
                                  ...
                                  <td><textarea id="suscrip" cols="61" rows="6"> </textarea><textarea name="suscriptores" cols="61" rows="6" style="display:none;"  oncopy="return false" onpaste="return false" oncut="return false"><?php echo (isset($newsletter)) ? htmlentities($newsletter['suscriptores']) : ""; ?></textarea><br>
                                    <button type="button" onclick='obtener_suscriptores(); pruebatext();' value="Insertar emails de suscriptores" >Insertar emails de los suscriptores a esta newsletter</button>
                                  </td>
                                </tr>
                              </table>
                            </td>
                                ...
                                <ul id="banners" style="padding-left:0px;margin-left:0px;padding-bottom:10px;background-color:#EEEEEE;border:1px solid #CCCCCC">
                                  ...
                                  if (mysql_num_rows($r) > 0) {
                                    while ($fila = mysql_fetch_assoc($r)) {
                                      ?>
                                      <li id="item_<?php echo $fila['id'] ?>,banners" style="<?php echo $estilos['banners_newsletter'] ?>" value="eo" title="Banners">
